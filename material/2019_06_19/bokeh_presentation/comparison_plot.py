...
def comparison_plot(
    ...
    """
    df, param_groups_and_heights, width = process_inputs(
        data_dict=data_dict,
    ...
    for param_group_name, group_height in param_groups_and_heights:
        to_plot = sorted(df[df["group"] == param_group_name]["full_name"].unique(), reverse=True)
        ...
        jittered_y = jitter("full_name", width=0.6, range=param_group_plot.y_range)
        ...
        _style_plot(param_group_plot, df)
...
def process_inputs(data_dict, color_dict, marker_dict, height, width):
    """
...
def _build_df_from_data_dict(data_dict, color_dict, marker_dict):
    ...
    for model, mod_dict in data_dict.items():
        ...
        ext_param_df["full_name"] = ext_param_df[name_cols].apply(
            lambda x: _index_element_to_string(tuple(x)), axis=1
        )
...
def _style_plot(fig, df):
    fig.xgrid.grid_line_color = None
    fig.yaxis.major_tick_line_color = None
    fig.yaxis.axis_line_color = "white"
    fig.outline_line_color = None
    fig.x_range=Range1d(0.95 * df['conf_int_lower'].min(), 1.05 * df['conf_int_upper'].max())
